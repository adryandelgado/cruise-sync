...
import { prefetchReturnsSession, ensureOnboardHub } from "@/lib/queryPrefetch";
import { isInitialQueryLoad } from "@/lib/queryLoading";

export const Route = createFileRoute("/_authed/onboard/returns/")({
  loader: ({ context: { queryClient } }) => ensureOnboardHub(queryClient),
  component: OnboardReturnsIndexPage,
});

function OnboardReturnsIndexPage() {
  const qc = useQueryClient();
  const { data: jobs, isPending } = useOnboardJobs();
  const loading = isInitialQueryLoad(isPending, jobs);
  const [search, setSearch] = useState("");
  const [filter, setFilter] = useState<OnboardJobFilter>("has_inventory");
  ...
  const filtered = useMemo(
    () => filterOnboardJobs(eligible, search, filter),
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      <header>
        <h1 className="text-2xl font-semibold tracking-tight">Returns & transfers</h1>
        <p className="text-sm text-stone-400">
          Pick a CSPO to build a return manifest or transfer items to another job.
        </p>
      </header>
      ...
      {!loading && eligible.length === 0 && (
        <Card className="p-6 text-sm text-stone-400">
          ...
          <p className="mt-2">
            Receive packages aboard first. Returns and transfers need trackable SKU items on
            vessel.
          </p>
        </Card>
      )}
      ...
      {!loading && eligible.length > 0 && filtered.length === 0 && (
        <Card className="py-12 text-center text-sm text-stone-500">
          No CSPOs match your filters — try “All” if inventory was cleared
        </Card>
      )}
      ...
      <div className="flex flex-col gap-3">
        {filtered.map((job) => (
          <Card
            ...
            onMouseEnter={() => prefetchReturnsSession(qc, job.cspo_id)}
          >
            <div>
